...
#'
#' @return \code{activation_curve} uses \code{cat()} to produce LaTeX code for drawing activation functions. Also, \code{activation_curve} saves the same output as a character vector invisibly, so users could use \code{cat()} to print it out later, as shown in Examples.
...
#' nnd_activation <- activation_curve()
